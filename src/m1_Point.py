...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        p = Point(old_x, old_y)
        self.moves += 1
        self.change = self.change + self.get_distance_from(p)
    ...
    def get_distance_from_start(self):
        self.distance = math.sqrt((self.dx - self.x)**2 + (self.dy - self.y)**2)
        return self.distance
    ...
    def halfway_to(self, p2):
        p1 = Point((self.x + p2.x)/2 ,(self.y + p2.y)/2)
        return p1
...
def run_test_move_to():
    ...
    """
    # ------------------------------------------------------------------
    # done: 6.  Follow the same instructions as in doneTODO 3 above,
